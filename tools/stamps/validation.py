...
from .rh_analyzer import RHIntegerAnalyzer, PascalKernel, GyroscopeLoss, QuantitativeGapAnalyzer
...
class NymanBeurlingStamp:
    ...
    def compute_shifted_dilation_approximation(depth: int, gamma: float) -> Tuple[float, int]:
        ...
        target = 1.0
        kernel_mean = sum(kernel_weights) / len(kernel_weights)
    ...
    def format_stamps_for_ce1(self, stamps: Dict[str, StampResult]) -> Dict[str, Any]:
        ...
        for name, stamp in stamps.items():
            stamps_dict[name] = {
                ...
                "depth": stamp.details.get("depth", 4),
